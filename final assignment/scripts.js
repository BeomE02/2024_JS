...
function updateSalesHistory() {
    ...
    salesHistoryDiv.innerHTML = ''; // 기존 내역 초기화
    ...
    sales.history.forEach((record, index) => {
        ...
        recordDiv.innerHTML = `
            <p><strong>${index + 1}.</strong> ${record.date}</p>
            <p>총 금액: ${record.total.toLocaleString()}원</p>
            <p>항목: ${record.items.map(item => `${item.name} x${item.quantity}`).join(', ')}</p>
        `;
